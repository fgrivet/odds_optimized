...
def epanechnikov_kernel(x, kc, bsi, bdsi):
    ...
    return np.array(
        [
            bdsi * np.prod([0.75 * (1 - value[i, j] ** 2) for j in range(value.shape[1])]) if (value[i] ** 2 <= 1).all() else 0
...
def neighbours_count(x, okc, bsi, bdsi, ws, ss, R):
    ...
    return (
        ...
        * np.sum(
            [
                np.prod(
                    (np.minimum(x + R, kc + B) - np.maximum(x - R, kc - B))
                    - (1 / 3)
                    * np.dot(
                        np.square(bsi), np.power(np.minimum(x + R, kc + B) - kc, 3) - np.power(np.maximum(x - R, kc - B) - kc, 3)
                    )
                )
                for kc in okc
            ]
...
class SDEM(SmartSifterSDEM):
    ...
    def fit(self, X, y=None):
        super().fit(X, y)
        self.covariances_bar = np.array(
            [
                (self.covariances_[i] + self.means_[i].reshape(-1, 1) @ self.means_[i].reshape(1, -1)) * self.weights_[i]
...
def optimized_monomials(x, n_matrix):
    x_repeated = np.tile(x, (n_matrix.shape[0], 1))
    return np.power(x_repeated, n_matrix)
...
def chebyshev_t_1_matrix(x, n_matrix):
    x_repeated = np.tile(x, (n_matrix.shape[0], 1))

    # Masques pour chaque condition sur x
    mask_1 = x_repeated < -1
    mask_2 = x_repeated > 1
    mask_3 = (x_repeated >= -1) & (x_repeated <= 1)

    # Initialisation du résultat
    result = np.zeros_like(n_matrix, dtype=float)

    # Calcul pour chaque condition
    # Cas x < -1
    result[mask_1] = (
        (-1) ** n_matrix[mask_1]
        * np.cosh(n_matrix[mask_1] * np.arccosh(-x_repeated[mask_1]))
        / np.where(n_matrix[mask_1] == 0, np.sqrt(np.pi), np.sqrt(np.pi / 2))
    )

    # Cas x > 1
    result[mask_2] = np.cosh(n_matrix[mask_2] * np.arccosh(x_repeated[mask_2])) / np.where(
        n_matrix[mask_2] == 0, np.sqrt(np.pi), np.sqrt(np.pi / 2)
    )

    # Cas -1 <= x <= 1
    result[mask_3] = np.cos(n_matrix[mask_3] * np.arccos(x_repeated[mask_3])) / np.where(
        n_matrix[mask_3] == 0, np.sqrt(np.pi), np.sqrt(np.pi / 2)
    )
...
def inverse_increment(mm, x, n, inv_opt, sym=True):
    N = x.shape[0]
    moments_matrix = n * mm.moments_matrix
    # Calcul de la matrice à ajouter
    V = np.empty((N, len(mm.monomials)), dtype=x.dtype)
    for idx, xx in enumerate(x):
        V[idx] = mm.polynomial_func(xx, mm.monomials)[:, 0]
...
def sherman_increment(mm, x, n, inv_opt, sym=True):
    """
    ...
    for xx in x:
        ...
        gauche = inv_moments_matrix @ v
        denom = v.T @ gauche
        gauche_div = gauche / (1 + denom)
        if sym == True:
            inv_moments_matrix -= gauche_div @ gauche.T
        else:
            droite = v.T @ inv_moments_matrix
            inv_moments_matrix -= gauche_div @ droite
    mm.inverse_moments_matrix = (n + x.shape[0]) * inv_moments_matrix


def woodbury_increment(mm, x, n, inv_opt, sym=True):
    """
    Identité de Woodbury
    (A + UCV)^-1 = A^-1 - A^-1 U (C^-1 + V A^-1 U)^-1 V A^-1
    Ici C est la matrice identité et U = V^T = [v_1, v_2, ..., v_N]
    """
    N = x.shape[0]

    # étape spécifique à Christoffel
    inv_moments_matrix = mm.inverse_moments_matrix / n
    ...
    if N == 1:
        ...
        somme_inv = inv_opt(somme)
    # A est symétrique est U = V^T donc A_inv_U = V_A_inv.T
    A_inv_U_somme_inv = V_A_inv.T @ somme_inv
    produit = A_inv_U_somme_inv @ V_A_inv

    inv_moments_matrix -= produit
...
def fpd_inv(M):  # according to: https://stackoverflow.com/a/58719188
    ...
    if info != 0:
        ...
        # Matrice probablement semi-définie positive, on essaie de la rendre définie positive
        # En ajoutant une régularisation de 1e-10 à 1e-4
        for eps in range(10, 3, -1):
            cholesky, info = lapack.dpotrf(M + 10**-eps * np.eye(M.shape[0]))
            # On s'arrête si la factorisation réussit
            if info == 0:
                break
            else:
                print(f"Error in dpotrf: {info} for eps = {eps}")
        # Si aucun n'a fonctionné, on lève une erreur
        if info != 0:
...
IMPLEMENTED_INVERSION_OPTIONS = {"inv": inv, "pinv": pinv, "pd_inv": pd_inv, "fpd_inv": fpd_inv}
...
class MomentsMatrix:
    def __init__(self, d, incr_opt="inverse", polynomial_basis="monomials", inv_opt="inv"):
        ...
        self.polynomial_func = lambda x, m: PolynomialsBasis.apply_combinations(
            x, m, IMPLEMENTED_POLYNOMIAL_BASIS[polynomial_basis]
        )
    ...
    def fit(self, x):
        ...
        X = np.empty((x.shape[0], len(monomials)), dtype=x.dtype)
        for idx, xx in enumerate(x):
            X[idx] = self.polynomial_func(xx, monomials)[:, 0]
        moments_matrix = np.dot(X.T, X) / len(x)
        self.moments_matrix = moments_matrix
        self.inverse_moments_matrix = self.inv_func(moments_matrix)
        return self

    def score_samples(self, x):
        v_matrix = np.array([self.polynomial_func(xx, self.monomials) for xx in x])[:, :, 0]
        temp = np.dot(v_matrix, self.inverse_moments_matrix)
    ...
    def update(self, x, n, sym=True):
        self.incr_func(self, x, n, self.inv_func, sym=sym)
        return self
...
class PolynomialsBasis:
    ...
    def generate_combinations(max_degree, dimensions):
        ...
        return sorted(combinations, key=lambda e: (np.sum(list(e)), list(-1 * np.array(list(e)))))
...
class RStarTreeNode:
    ...
    def __chose_split_axis__(self):
        ...
        for i in range(self.low.shape[1]):
            ...
            for j in range(self.max_size - 2 * self.min_size + 2):
                ...
                fg_margin = np.prod(
                    np.max([r.high for r in first_group], axis=0) - np.min([r.low for r in first_group], axis=0)
                ) - np.sum([r.__compute_volume__() for r in first_group])
                second_group = sorted_entries[self.min_size + j :]
                sg_margin = np.prod(
                    np.max([r.high for r in second_group], axis=0) - np.min([r.low for r in second_group], axis=0)
                ) - np.sum([r.__compute_volume__() for r in second_group])
    ...
    def __chose_split_index__(self, split_axis) -> (int, list, list):
        ...
        for j in range(self.max_size - 2 * self.min_size + 2):
            ...
            overlap_volume = np.prod(
                np.maximum(np.zeros(fg_low.shape), np.minimum(fg_high, sg_high) - np.maximum(fg_low, sg_low))
            )
    ...
    def __update_k_dist__(self, obj):
        if obj == self.max_k_dist[1]:
            if self.max_k_dist[0] < obj.__dict__["__k_dist__"]:  # The max k-distance has increased and needs to be updated
                ...
                self.max_k_dist[0] > obj.__dict__["__k_dist__"]
            ):  # The max k-distance has decreased and needs to be chosen again and updated
                if self.level == self.leaf_level:
                    self.max_k_dist = sorted(
                        [[o.__dict__["__k_dist__"], o] for o in self.children if o.__dict__.get("__k_dist__") is not None],
                        key=lambda elt: -1 * elt[0],
                    )[0]
                else:
...
def multiprocessable_compute_method(method, savename, x_train, x_test, y_test, inf_replacement: float = 1e-9):
    if type(method) == dict:
        ...
        try:
            ...
            y_decision[y_decision == -np.inf] = (
                -inf_replacement
            )  # the used metrics do not work with inf values that can be obtained with iLOF
...
def load_dataset(filename):
    ...
    data[:, :-1] = 2 * (((data[:, :-1] - vmin) / (vmax - vmin)) - 0.5)
    return data
...
def precision(y_score, score_outliers, threshold):
    ...
    return (
        len(score_outliers[score_outliers < threshold]) / len(y_score[y_score < threshold])
        if len(y_score[y_score < threshold]) != 0
        else 1
    )
...
def roc_and_pr_curves(y_true, y_score):
    ...
    prec = np.array(
        [
            len(score_outliers[score_outliers < s]) / len(y_score[y_score < s]) if len(y_score[y_score < s]) != 0 else np.nan
            for s in thresholds
        ]
    )
